...
outfile = paste(timeStamp, '/', timeStamp, '/', sub_dir_sim, '/report_', nameA, '_', nameB, '.txt', sep='')
outfile_best = paste(timeStamp, '/', timeStamp, '/', sub_dir_sim, '/best_model.txt', sep='')
...
# observed data
ss = c(1, 4:11, 14:17, 19:22, 24:31, 40:41)
obs_ss = read.table(paste(timeStamp, '/', timeStamp, '/ABCstat_global.txt', sep=''), h=T)[, ss] # global statistics over all loci (avg and std)
obs_sfs = read.table(paste(timeStamp, '/', timeStamp, '/ABCjsfs.txt', sep=''), h=T)
...
write.table(sfs, paste(timeStamp, '/', timeStamp, '/sfs_table.txt', sep=''), col.names=T, row.names=T, sep='\t', quote=F)

sfs[1,2]=0; sfs[2,1]=0 # remove the singletons, ONLY FOR THE REPRESENTATION
pdf(paste(timeStamp, '/', timeStamp, '/sfs_plot.pdf', sep=''), bg="white")
...
for(m in models){
	...
	for(rep in seq(0, nSubdir-1, 1)){
		...
		if(file.exists(paste(timeStamp, '/', timeStamp, '/', sub_dir_sim, '/', m, '_', rep, '/priorfile.txt', sep='')) == TRUE){
			# statistics
			tmp_ss = read.table(paste( timeStamp, '/', timeStamp, '/', sub_dir_sim, '/', m, '_', rep, '/ABCstat.txt', sep=''), h=T)[, ss]
			
			tmp_sfs = read.table(paste( timeStamp, '/', timeStamp, '/', sub_dir_sim, '/', m, '_', rep, '/ABCjsfs.txt', sep=''), h=T)
			tmp = cbind(tmp_ss, tmp_sfs)
			
			ss_sim_tmp = rbind(ss_sim_tmp, tmp)
			
			# params
			tmp_params = read.table(paste( timeStamp, '/', timeStamp, '/', sub_dir_sim, '/', m, '_', rep, '/priorfile.txt', sep=''), h=T)
...
mod_iso_mig = abcrf(as.factor(modIndexes)~., data = data.frame(modIndexes, all_models_sim[, -ss_2_remove]), ntree = ntree, paral = T, ncores = ncores)
...
if(predicted_model_iso_mig$allocation=='migration'){
	...
	#model comparison	
	mod = abcrf(as.factor(modIndexes)~., data = data.frame(modIndexes, all_models[, -ss_2_remove]), ntree = ntree, paral = T, ncores = ncores)
	...
	mod_Mhomo_Mhetero = abcrf(as.factor(modIndexes)~., data = data.frame(modIndexes, rbind(Mhomo, Mhetero)[, -ss_2_remove]), ntree = ntree, paral = T, ncores = ncores)
	...
	mod_Nhomo_Nhetero = abcrf(as.factor(modIndexes)~., data = data.frame(modIndexes, rbind(Nhomo, Nhetero)[, -ss_2_remove]), ntree = ntree, paral = T, ncores = ncores)
...
if(predicted_model_iso_mig$allocation=='isolation'){
	...
	if( predicted_model$allocation == 'SI' ){
		...
		mod_best = abcrf(as.factor(modIndexes)~., data = data.frame(modIndexes, rbind(SI_1N, SI_2N)[, -ss_2_remove]), ntree = ntree, paral = T, ncores = ncores)
		...
		mod_best = abcrf(as.factor(modIndexes)~., data = data.frame(modIndexes, rbind(AM_1M_1N, AM_1M_2N, AM_2M_1N, AM_2M_2N)[, -ss_2_remove]), ntree = ntree, paral = T, ncores = ncores)
...
# summarized output
summary_outfile = paste( timeStamp, '/', timeStamp, '/', sub_dir_sim, '/hierarchical_models.txt', sep='')
