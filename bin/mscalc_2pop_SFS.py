		...
		if segsites != 0:
			if "positions" not in line and line!="\n":
				...
				if nSam_cnt == (nSamA[nLoci_cnt - 1] + nSamB[nLoci_cnt - 1]):
					...
					freqA = tmpA['freq']
					piA.append(tmpA['pi']/nSNPs[nLoci_cnt - 1])
					
					tmpB = compFreq(spB, segsites)
					freqB = tmpB['freq']
					piB.append(tmpB['pi']/nSNPs[nLoci_cnt - 1])
					
					tmp = sites(freqA, freqB, segsites) # determines the # of sxA, sxB, ss, sf
					sf.append(tmp['sf']/(1.0*nSNPs[nLoci_cnt - 1]))
					sxA.append(tmp['sxA']/(1.0*nSNPs[nLoci_cnt - 1]))
					sxB.append(tmp['sxB']/(1.0*nSNPs[nLoci_cnt - 1]))
					ss.append(tmp['ss']/(1.0*nSNPs[nLoci_cnt - 1]))
					...
					thetaA.append(thetaA_locus/nSNPs[nLoci_cnt - 1 ])
					thetaB.append(thetaB_locus/nSNPs[nLoci_cnt - 1 ])
					...
					divAB.append(div['divAB']/nSNPs[nLoci_cnt - 1 ])
					netdivAB.append(div['divAB']/nSNPs[nLoci_cnt - 1 ] - (tmpA['pi']/nSNPs[nLoci_cnt - 1] + tmpB['pi']/nSNPs[nLoci_cnt - 1]) / 2.0)
					minDivAB.append(div['minDivAB']/nSNPs[nLoci_cnt - 1 ])
					maxDivAB.append(div['maxDivAB']/nSNPs[nLoci_cnt - 1 ])
